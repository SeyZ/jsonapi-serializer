...
module.exports = function (collectionName, payload, opts) {
  ...
  function getLinks(currentResult, record, links) {
    return _.mapValues(links, function (value) {
      if (_.isFunction(value)) {
        return value(currentResult, record);
      } else {
        return value;
      }
    });
      ...
      if (opts && opts.ref) {
        ...
        if (opts.relationshipLinks) {
          dest.relationships[dasherize(attribute)].links =
            getLinks(dest, record[attribute], opts.relationshipLinks);
        }
      } else {
  ...
  this.serializeRef = function (dest, record, attribute, opts) {
    ...
    if (_.isUndefined(opts.included) || opts.included) {
      if (opts.includedLinks) {
        included.links = getLinks(dest, dest, opts.includedLinks);
      }
  ...
  this.perform = function (record) {
    ...
    // Data links.
    if (opts.dataLinks) { data.links = getLinks(data, record, opts.dataLinks); }
