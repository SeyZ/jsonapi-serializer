...
module.exports = function (collectionName, record, payload, opts) {
  ...
  this.serializeRef = function (dest, current, attribute, opts) {
    ...
    relationships.forEach(function (relationship) {
      ...
      } else if (dest) { // if dest is null this is a "null relationship" so skip
        var data = that.serializeRef(dest[relationship], included, relationship, opts[relationship])

        if (!included.relationships) { included.relationships = {}; }

        included.relationships[keyForAttribute(relationship)] = {data: data}
      }
    });
