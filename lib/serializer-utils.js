...
module.exports = function (collectionName, record, payload, opts) {
  ...
  function getType(str, attrVal,opt) {
    var type;
    attrVal = attrVal || {};
    if (opt) {
      type = opt.type;
      if(_.isUndefined(type) && opt && _.isFunction(opt.typeForAttribute)) {
        type = opt.typeForAttribute(str, attrVal);
      }
    }

    if (_.isUndefined(type) && _.isFunction(opts.typeForAttribute)) {
  ...
  function isComplexType(obj) {
    return _.isArray(obj) || _.isPlainObject(obj) || _.isString(obj);
  }
