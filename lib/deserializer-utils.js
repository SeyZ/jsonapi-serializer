...
module.exports = function(jsonapi, data, opts) {
    ...
    return Promise.all(
      Object.keys(from.relationships).map(function(key) {
        ...
        if (relationship.data === null) {
          ...
          return extractIncludes(relationship.data, key, from).then(function(
            include
          ) {
            if (include) {
  ...
  function extractIncludes(relationshipData, relationshipName, from) {
    return findIncluded(relationshipData, relationshipName, from).then(function(
      included
    ) {
      var valueForRelationship = getValueForRelationship(
        relationshipData,
        included
      );
  ...
  function deserializeRecord(data) {
    ...
    return Promise.all(thingsToBeExtracted).then(function(results) {
      var attributes = results[0];
      var relationships = results[1];
      if (results[2]) {
        var links = results[2];
